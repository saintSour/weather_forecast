...
void WeatherAnimationWidget::initializeScene() {
  ...
  switch (condition_) {
    ...
    case WeatherCondition::Unknown:
      clouds_.resize(condition_ == WeatherCondition::PartlyCloudy ||
...
void WeatherAnimationWidget::tick() {
  ...
  for (auto &particle : particles_) {
    ...
    if (particle.x < 0.0F) particle.x = width;
  ...
  if (condition_ == WeatherCondition::Thunderstorm &&
      QRandomGenerator::global()->bounded(180) == 0)
...
void WeatherAnimationWidget::drawSun(QPainter &painter) {
  ...
  for (int ray = 0; ray < 8; ++ray) {
    ...
    const QPointF start =
        center + QPointF(qCos(angle) * 28.0, qSin(angle) * 28.0);
    const QPointF end =
        center + QPointF(qCos(angle) * 38.0, qSin(angle) * 38.0);
...
void WeatherAnimationWidget::drawClouds(QPainter &painter) {
  ...
  for (const Cloud &cloud : clouds_) {
    painter.setBrush(
        QColor(255, 255, 255, static_cast<int>(cloud.opacity * 255.0F)));

    const QRectF base(cloud.x, cloud.y, cloud.width, cloud.height);
    painter.drawEllipse(base);
    painter.drawEllipse(
        base.translated(-cloud.width * 0.28, cloud.height * 0.18));
    painter.drawEllipse(
        base.translated(cloud.width * 0.28, cloud.height * 0.12));
  }
}
...
void WeatherAnimationWidget::drawPrecipitation(QPainter &painter) {
  if (condition_ == WeatherCondition::Snow) {
    ...
    for (const Particle &particle : particles_)
      painter.drawEllipse(QPointF(particle.x, particle.y), particle.size,
                          particle.size);
  ...
  for (const Particle &particle : particles_) {
    painter.drawLine(QPointF(particle.x, particle.y),
                     QPointF(particle.x + particle.drift * 4.0F,
                             particle.y + particle.size * 5.0F));
  }
}
...
void WeatherAnimationWidget::drawFog(QPainter &painter) {
  ...
  for (int layer = 0; layer < 3; ++layer) {
    const float y = height() * (0.45F + layer * 0.12F) +
                    qSin(animationPhase_ + layer) * 6.0F;
    painter.drawRect(QRectF(-10.0, y, width() + 20.0, 28.0));
  }
}

void WeatherAnimationWidget::drawLightning(QPainter &painter) {
  if (lightningFlash_ <= 0.0F) return;

  painter.fillRect(rect(), QColor(255, 255, 255,
                                  static_cast<int>(lightningFlash_ * 120.0F)));
